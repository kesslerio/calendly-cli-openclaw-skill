...
const embeddedSchemas = {
  ...
  "reschedule_event": {
    ...
    "properties": {
      "event_uuid": {
        "type": "string",
        "description": "UUID of the scheduled event to reschedule"
      },
      "invitee_uuid": {
        "type": "string",
        "description": "UUID of the invitee associated with the event (alternative identifier)"
      },
      "reschedule_url": {
        "type": "string",
        "description": "Calendly reschedule URL (alternative identifier source)"
      },
      "new_start_time": {
        "type": "string",
        "description": "New start time for the event (ISO 8601 format)"
      },
      "new_end_time": {
        "type": "string",
        "description": "Optional explicit new end time (ISO 8601 format)"
      },
      "event_type": {
        "type": "string",
        "description": "Optional event type URI override for the new slot"
      },
      "reason": {
        "type": "string",
        "description": "Optional reason shown in reschedule notifications"
      }
    },
    "required": [
      "new_start_time"
...
const generatorTools = [
  ...
  },
  {
    ...
    "usage": "list-events [--user-uri <user-uri>] [--organization-uri <organization-uri>] [--status <status:active|canceled>] [--max-start-time <max-start-time:iso-8601>] [--min-start-time <min-start-time:iso-8601>] [--include-invitees] [--expand <expand:invitees>] [--hydrate-invitees <hydrate-invitees:boolean>] [--max-invitee-fetches <max-invitee-fetches:number>] [--raw <json>]",
    "flags": "[--user-uri <user-uri>] [--organization-uri <organization-uri>] [--status <status:active|canceled>] [--max-start-time <max-start-time:iso-8601>] [--min-start-time <min-start-time:iso-8601>] [--include-invitees] [--expand <expand:invitees>] [--hydrate-invitees <hydrate-invitees:boolean>] [--max-invitee-fetches <max-invitee-fetches:number>] [--raw <json>]"
  },
  {
    "name": "list-events-with-invitees",
    "description": "Compatibility command for listing events with invitee details in one API call",
    "usage": "list-events-with-invitees [--user-uri <user-uri>] [--organization-uri <organization-uri>] [--status <status:active|canceled>] [--max-start-time <max-start-time:iso-8601>] [--min-start-time <min-start-time:iso-8601>] [--count <count:number>] [--hydrate-invitees <hydrate-invitees:boolean>] [--max-invitee-fetches <max-invitee-fetches:number>] [--raw <json>]",
  ...
  },
  {
    ...
    "description": "Reschedule an existing scheduled event to a new start time",
    "usage": "reschedule-event (--event-uuid <event-uuid> | --invitee-uuid <invitee-uuid> | --reschedule-url <reschedule-url>) --new-start-time <new-start-time:iso-8601> [--event-type <event-type-uri>] [--new-end-time <new-end-time:iso-8601>] [--reason <reason>] [--event-uri <event-uri>] [--invitee-uri <invitee-uri>] [--raw <json>]",
    "flags": "(--event-uuid <event-uuid> | --invitee-uuid <invitee-uuid> | --reschedule-url <reschedule-url>) --new-start-time <new-start-time:iso-8601> [--event-type <event-type-uri>] [--new-end-time <new-end-time:iso-8601>] [--reason <reason>] [--event-uri <event-uri>] [--invitee-uri <invitee-uri>] [--raw <json>]"
  },
  {
    "name": "get-event",
...
const embeddedMetadata = {
  ...
  "invocation": {
    ...
    "runtime": "bun",
    "bundler": "bun",
    "outputPath": "src/generated/cli.ts",
    "timeoutMs": 30000,
    "minify": false
  }
} as const;
...
const commandSignatures: Record<string, string> = {
  ...
  "list-events": "function list_events(user_uri?: string, organization_uri?: string, status?: \"active\" | \"canceled\", max_start_time?: string, min_start_time?: string, include_invitees?: boolean, expand?: string, count?: number, hydrate_invitees?: boolean, max_invitee_fetches?: number);",
  ...
  "list-team-events": "function list_team_events(organization_uri: string, status?: \"active\" | \"canceled\", min_start_time?: string, max_start_time?: string, count?: number, max_membership_pages?: number, member_email?: string, member_uri?: string, event_type_name?: string, include_invitees?: boolean, hydrate_invitees?: boolean, max_invitee_fetches?: number);",
  ...
  "reschedule-event": "function reschedule_event(new_start_time: string, event_uuid?: string, invitee_uuid?: string, reschedule_url?: string, new_end_time?: string, event_type?: string, reason?: string);",
...
async function fetchBookingInviteeResource(inviteeUuid: string, timeout: number): Promise<Record<string, unknown>> {
	const response = await axios.get(`https://calendly.com/api/booking/invitees/${encodeURIComponent(inviteeUuid)}`, {
		headers: {
			'Content-Type': 'application/json',
		},
		timeout,
	});
	return toObjectRecord(response.data);
}

function extractInviteeUuid(value: unknown): string | undefined {
	const record = toObjectRecord(value);
	const inviteeUri = pickStringValue(
		record.uri,
		record.invitee_uri,
		toObjectRecord(record.invitee).uri
	);
...
async function resolveRescheduleEventQuery(
	...
	if (!inviteeUuid) {
		throw new Error('Unable to determine invitee_uuid for rescheduling. Provide --invitee-uuid/--invitee-uri, --reschedule-url, or an event with invitees.');
	}
...
async function fetchRescheduleEventResult(query: Record<string, unknown>, timeout: number): Promise<any> {
	const apiKey = process.env.CALENDLY_API_KEY;
	if (!apiKey) throw new Error('CALENDLY_API_KEY environment variable is required');
	...
	const response = await axios.patch(
		...
		toRescheduleEventRestBody(resolvedQuery as any),
		{
			headers: {
				'Content-Type': 'application/json',
	...
	return shapeRescheduleEventResult(response.data, resolvedQuery as any);
}
...
program
	...
	.option("--hydrate-invitees <hydrate-invitees:boolean>", "Hydrate missing invitees with per-event fallback calls (default true when invitees are included)", parseBooleanFlag)
	.option("--max-invitee-fetches <max-invitee-fetches:number>", "Safety cap for per-event invitee fallback API calls (default 25)", (value) => parseInt(value, 10))
	.action(async (cmdOpts) => {
		const globalOptions = program.opts();
		try {
			...
			const resultData = await scanTeamEvents(query, {
				...
				fetchEventInviteesPage: async (eventUuid: string, pageToken?: string) => {
					...
					params.append('count', '100');
					if (pageToken) params.append('page_token', pageToken);
					const response = await axios.get(`https://api.calendly.com/scheduled_events/${eventUuid}/invitees?${params.toString()}`, {
						headers: {
							'Authorization': `Bearer ${apiKey}`,
							'Content-Type': 'application/json',
						},
...
program
	...
	.action(async (cmdOpts) => {
		...
		try {
			...
			try {
				const call = (proxy.rescheduleEvent as any)(toRescheduleEventMcpArgs(query));
				const mcpResult = await invokeWithTimeout(call, timeout);
				const mcpRaw = createCallResult(mcpResult).raw as Record<string, unknown> | undefined;
